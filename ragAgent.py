...
def store_memory(question: BaseMessage, config: RunnableConfig, store: BaseStore) -> str:
    ...
    try:
        ...
        memories = store.search(namespace, query=str(question.content))
        # 修复：过滤掉 None 值
        valid_memories = [d.value["data"] for d in memories if d.value.get("data") is not None]
        user_info = "\n".join(valid_memories)
...
def agent(state: MessagesState, config: RunnableConfig, *, store: BaseStore, llm_chat, tool_config: ToolConfig) -> dict:
    ...
    try:
        ...
        if not state.get("messages") or len(state["messages"]) == 0:
            ...
            return {"messages": [{"role": "system", "content": "状态异常，无法处理请求"}]}
        ...
        # 检查问题内容是否有效
        if not hasattr(question, "content") or not question.content:
            logger.error("Invalid question content")
            return {"messages": [{"role": "system", "content": "无效的问题内容"}]}
        ...
        try:
            ...
            logger.error(f"Error invoking agent chain: {invoke_error}")
            return {"messages": [{"role": "system", "content": "模型服务暂时不可用，请稍后重试"}]}

    except Exception as e:
        logger.error(f"Error in agent processing: {e}")
        return {"messages": [{"role": "system", "content": "处理请求时出错，请稍后重试"}]}
...
def main():
    ...
    try:
        ...
        llm_chat, a = get_llm(Config.LLM_TYPE)
        b, llm_embedding=get_llm("ollama")
        # 获取工具列表
